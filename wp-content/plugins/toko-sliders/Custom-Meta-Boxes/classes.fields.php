	...
	public function get_default_args() {
		return apply_filters(
			...
			array(
				...
				'data_delegate'       => null,
				'save_callback'       => null,
				'string-repeat-field' => __( 'Add New', 'tp-slider' ),
				'string-delete-field' => __( 'Remove Field', 'tp-slider' ),
			),
			get_class( $this )
		);
	}
	...
	public function html() {
		...
		<div class="cmb-file-wrap" <?php echo 'data-type="' . esc_attr( $data_type ) . '"'; ?>>
			...
			<button class="button cmb-file-upload <?php echo esc_attr( $this->get_value() ) ? 'hidden' : '' ?>">
				<?php esc_html_e( 'Add File', 'tp-slider' ); ?>
			</button>
			...
			<button class="cmb-remove-file button <?php echo $this->get_value() ? '' : 'hidden'; ?>">
				<?php esc_html_e( 'Remove', 'tp-slider' ); ?>
			</button>
	...
	public function html() {
		...
		<div class="cmb-file-wrap" style="<?php echo esc_attr( $styles ); ?>" data-type="<?php echo esc_attr( $data_type ); ?>">
			...
			<button class="button cmb-file-upload <?php echo esc_attr( $this->get_value() ) ? 'hidden' : '' ?>" data-nonce="<?php echo wp_create_nonce( 'cmb-file-upload-nonce' ); ?>">
				<?php esc_html_e( 'Add Image', 'tp-slider' ); ?>
			</button>
...
class CMB_Select extends CMB_Field {
	...
	public function output_field() {
		...
		$none = is_string( $this->args['allow_none'] ) ? $this->args['allow_none'] : __( 'None', 'tp-slider' );
	...
	public function output_script() {
		...
		$options = wp_parse_args( $this->args['select2_options'], array(
			'placeholder' => __( 'Type to search', 'tp-slider' ),
			'allowClear'  => true,
		) );
...
class CMB_Group_Field extends CMB_Field {
	...
	public function get_default_args() {
		return array_merge(
			...
			array(
				'fields'              => array(),
				'string-repeat-field' => __( 'Add New Group', 'tp-slider' ),
				'string-delete-field' => __( 'Remove Group', 'tp-slider' ),
			)
		);
	}
	...
	public function get_default_args() {
		return array_merge(
			...
			array(
				...
				'default_zoom'        => '8',
				'string-marker-title' => __( 'Drag to set the exact location', 'tp-slider' ),
			)
		);
	}
