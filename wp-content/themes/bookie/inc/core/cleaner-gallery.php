<?php
/**
 * Cleaner Gallery For BootStrap - modified from Cleaner Gallery.
 *
 * @package WordPress
 * @subpackage Toko
 *
...
add_filter( 'post_gallery', 'toko_cleaner_gallery', 2002, 2 );
 ...
 * @return string
 */
function toko_cleaner_gallery( $output, $attr ) {
	...
	foreach ( $attachments as $attachment ) {
		...
		if ( $columns > 0 && $i % $columns == 0 )
			$output .= "\n\t\t\t\t<div class='row gallery-row gallery-col-{$columns} gallery-clear'>";

		/* Open each gallery item. */
		$output .= "\n\t\t\t\t\t<{$itemtag} class='col-sm-{$grid} gallery-item'>";
		...
		$output .= "\n\t\t\t\t\t\t<{$icontag} class='thumbnail gallery-icon {$orientation}'>";
		...
		if ( !empty( $caption ) )
			$output .= "\n\t\t\t\t\t\t<{$captiontag} class='caption gallery-caption'>{$caption}</{$captiontag}>";
